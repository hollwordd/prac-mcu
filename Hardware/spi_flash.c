...
//汉字库写的地址
void SPI_FLASH_WriteHzku(u32 num)	//261,696
{
	//写数据之前，先进行擦除一块空间，汉字库的大小
	int i;
	u8 ch;
	printf("请从串口获取一个字符e进行擦除，其他字符擦除：");
	ch = Usart_Rec(USART1);
	if(ch == 'e')
	{
		for(i=0;i<num;i+=4096)
		{
			SPI_FLASH_SectorErase(i);
		}
		printf("擦除成功,请发送一个.bin文件\r\n");
	}
	for(i=0;i<num;i++)
	{
		ch = Usart_Rec(USART1);
		SPI_FLASH_BufferWrite(&ch,i,1);
	}
	printf("文件写入成功！\r\n");
}

//验证数据是否写入成功
//从flash中获取字符的数据，需要计算字符在flash中的位置
//16*16/8 = 32个字节 
//GB2312 --> 占两个字节
//pos = ((Buf[0]-0xA1)*94 + Buf[1]-0xA1)*32 + [flash存放位置]
//从flash中读取数据SPI_FLASH_BufferRead();
void SPI_FLASH_ReadHzbuf(u16 x,u16 y,u16 fcolor,u16 bcolor,u8 *Hzbuf)
{
	int pos;
	unsigned char Fontbuf[32] = {'\0'};
	pos = ((Hzbuf[0]-0xA1)*94 + Hzbuf[1]-0xA1)*32;
	SPI_FLASH_BufferRead(Fontbuf,pos,32);
	Lcd_DisplayAnySizeChar(x,y,16,16,fcolor,bcolor,Fontbuf);
}
